...
class Graph(width: Int = 100, length: Int = 30) {
  ...
  val pointsSet = mutable.HashMap[Char, mutable.ListBuffer[Point]]()
  ...
  private def markOptimizationSpace(original: Char, simulated: Char): Unit = {

    (0 to width).foreach (x => {
      breakable {
        (0 to length).foreach(y => {
          if (graphString(y)(x) == simulated) break
          graphString(y)(x) = '/'
        })
      }
    })
  ...
  private def plot(c: Char): Unit = {
    //println(s"plotting for $c")

    //val points = compressedPointsSet(c)
    // this is probably not needed
    //val sortedPoints = points.sortWith(_.x < _.x)
  ...
  private def largerGraph(sortedPoints: ListBuffer[Point], xJump: Long): Unit = {
    ...
    while (!qu.isEmpty) {
      ...
      if (nextTarget > front.x || lastPoint == null) {
        //println(s"1st: adding point (${index}, ${front.y})")
        //fill the graph
        if (lastPoint == null) {
        ...
        //println(s"extrapolation points = ${ep}")
        val yJumps = (front.y - lastPoint.y) / ep
        var lastY = lastPoint.y
        (1 to ep).foreach(i => {
          //println(s"2nd: adding point (${index}, ${lastPoint.y + (i * yJumps)})")
      ...
      //println(s"new indxe = ${index}, nextTarget = ${nextTarget}")
    }
  }
