...
class PQParallelStageScheduler(var totalCores: Int, taskCountMap: mutable.HashMap[Int, Int],
                               _delayedExecutors: List[DelayedExecutors], coresPerExecutor: Int = 0)
  extends
  TaskScheduler {
  val NO_STAGE_ID = -1
  val delayedExecutors: mutable.Queue[DelayedExecutors] =
    scala.collection.mutable.Queue(_delayedExecutors: _*)
  ...
  override def schedule(taskTime: Int, stageID: Int): Unit = {
    if (!delayedExecutors.isEmpty && !taskQueue.isEmpty) {
      val task = taskQueue.head
      if (task.finishingTime > delayedExecutors.head.time) {
        val delayed = delayedExecutors.dequeue()
        totalCores += (delayed.numExecutors * coresPerExecutor)

//        println(s"ADDING ${delayed.numExecutors} at time ${pd(delayed.time)} for stage = " +
 //         s"${stageID}")
      }
    }
    ...
    //if (stageID == 0) println(s"task enqueu at time ${pd(wallClock)}. Size = ${taskQueue.size}")
  }
